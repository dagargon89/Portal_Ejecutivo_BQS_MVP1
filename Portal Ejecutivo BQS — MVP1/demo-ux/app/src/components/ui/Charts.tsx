/* =====================================================================
 * Charts.tsx — Gráficas ligeras del demo (SVG/CSS, sin dependencias).
 * Consumen datos ya calculados por el adaptador (api.dashMetricas); el
 * cliente solo grafica. Colores desde los tokens de marca (doc 08) vía
 * var(--color-*). Cada gráfica incluye texto alterno para accesibilidad.
 * ===================================================================== */
import type { EstatusPago, SerieMes, DistribEstatus, TopClienteCartera, EmbudoCiclo } from "@/lib/types";
import { money, moneyCompact, mesCorto, num } from "@/lib/format";

const ESTATUS_COLOR: Record<EstatusPago, string> = {
...
export function BarrasMensuales({ data }: { data: SerieMes[] }) {
  ...
  return (
    <figure className="m-0">
      <div className="flex h-44 items-end gap-2 sm:gap-4" role="img" aria-label="Facturado contra cobrado por mes (últimos 6 meses)">
        {data.map((d) => (
          <div key={d.periodo} className="flex flex-1 flex-col items-center gap-2">
            <div className="flex h-full w-full items-end justify-center gap-1">
              <div
                ...
                title={`Facturado ${mesCorto(d.periodo)}: ${money(d.facturado)}`}
              />
              <div
                className="w-3 rounded-t bg-secondary transition-all sm:w-4"
                style={{ height: `${(d.cobrado / max) * 100}%` }}
                title={`Cobrado ${mesCorto(d.periodo)}: ${money(d.cobrado)}`}
              />
            </div>
...
export function DonaEstatus({ data }: { data: DistribEstatus[] }) {
  ...
  return (
    <div className="flex flex-col items-center gap-5 sm:flex-row sm:justify-center sm:gap-8">
      <svg viewBox="0 0 160 160" className="h-40 w-40 shrink-0 -rotate-90" role="img" aria-label="Distribución de facturas por estatus">
        ...
        {data.map((d) => {
          ...
          const seg = (
            ...
            >
              <title>{`${d.estatus}: ${money(d.monto)} (${d.cantidad})`}</title>
            </circle>
          );
          offset += dash;
          return seg;
      ...
      <ul className="flex flex-col gap-2 text-sm">
        {data.map((d) => (
          <li key={d.estatus} className="flex items-center gap-2">
            <span className="h-3 w-3 rounded-sm" style={{ backgroundColor: ESTATUS_COLOR[d.estatus] }} aria-hidden />
            <span className="font-medium text-slate-700">{d.estatus}</span>
            <span className="text-slate-400">·</span>
            <span className="num text-slate-500">{money(d.monto)}</span>
        ...
        <li className="mt-1 border-t border-slate-100 pt-2 text-xs text-slate-400">
          {num(totalCant)} facturas en total
        </li>
      </ul>
    </div>
  );
      ...
      {data.map((d) => (
        <li key={d.ID_Cliente}>
          <div className="mb-1 flex items-center justify-between gap-2 text-sm">
            <span className="truncate font-medium text-slate-700">{d.Nombre_Comercial ?? d.ID_Cliente}</span>
            <span className="num shrink-0 text-slate-500">{money(d.saldo)}</span>
          </div>
          <div className="h-2.5 w-full overflow-hidden rounded-full bg-slate-100">
            <div className="h-full rounded-full bg-primary" style={{ width: `${(d.saldo / max) * 100}%` }} aria-hidden />
          </div>
        </li>
      ))}
    </ul>
        ...
        return (
          <li key={key} className="flex items-center gap-3">
            ...
            <div className="flex h-8 flex-1 items-center rounded-md bg-slate-100">
              ...
              >
                <span className="num whitespace-nowrap text-xs font-semibold text-white">{moneyCompact(valor)}</span>
              </div>
            </div>
